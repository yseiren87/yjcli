...
description: >-
  ...
  For browser-extension/native_*/**, use dj-backend-service instead. Do not use
  for frontend/, mobile-app/, pc-app/, backend/, backend-service/, or cli/.
---

# dj-browser-extension

Requires `dj-arch-core`. Scope: **`browser-extension/`** excluding **`native_*/`**.
...
browser-extension/
  ...
  {extension_name}/
    ...
    src/
      ...
      popup/          # view
      lib/            # infra
  native_{name}/      # NOT this skill → dj-backend-service
```
...
- Architecture: **`dj-backend-service`** (single local process).
- When editing `native_*`, do not apply extension UI rules; load `dj-backend-service`.
...
- Companion host changes → switch to `dj-backend-service`.
