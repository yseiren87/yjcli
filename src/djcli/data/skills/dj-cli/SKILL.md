...
Requires `dj-arch-core`. Scope: **`cli/` only**.
...
- `cli/{app_name}/` only.
- Native messaging hosts that belong to extensions live under `browser-extension/native_*` and use `dj-backend-service`, not this skill — unless the user places a pure CLI under `cli/`.
